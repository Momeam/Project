...
import { User, Phone, MessageSquare, Mail, MapPin, ArrowLeft, CheckCircle, Share2, Heart, Bed, Bath, Ruler } from 'lucide-react';
...
export default function ListingDetailPage() {
    ...
    const MapDisplay = useMemo(() => dynamic(
        ...
        { 
            loading: () => <div className="h-[400px] w-full bg-slate-50 rounded-2xl animate-pulse" />,
            ssr: false 
        }
    ), []);
    ...
    const seller = useMemo(() => {
        ...
        return users.find(u => u.id === property.userId);
    }, [property, users]);

    const contactInfo = {
        name: seller?.verificationDetails?.fullName || seller?.username || 'ไม่ระบุชื่อ',
        phone: seller?.verificationDetails?.phoneNumber || property?.contact?.phoneNumber || '-',
        line: seller?.verificationDetails?.lineId || '-',
        email: seller?.verificationDetails?.email || seller?.email || '-',
        image: seller?.verificationDetails?.documentUrl 
    };
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="space-y-4">
                            <div className="aspect-video bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 relative">
                                <img src={activeImage || 'https://placehold.co/800x600'} alt={property.title} className="w-full h-full object-contain bg-slate-50" />
                        ...
                        <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-6 border-t border-b border-slate-100">
                                {[
                                    { label: 'ห้องนอน', val: property.bedrooms, icon: Bed },
                                    { label: 'ห้องน้ำ', val: property.bathrooms, icon: Bath },
                                    { label: 'พื้นที่', val: `${property.size} ตร.ม.`, icon: Ruler },
                            ...
                            <CardContent className="p-6 relative">
                                ...
                                <div className="mt-14 text-center mb-6">
                                    <h3 className="text-xl font-bold text-slate-900 flex items-center justify-center gap-1">
                                        {contactInfo.name}
                                        {seller?.verificationStatus === 'APPROVED' && <CheckCircle className="w-4 h-4 text-blue-500" />}
                                    </h3>
                                    <p className="text-sm text-slate-500">ID: {seller?.id || property.userId}</p>
                                </div>
                                ...
                                <Button className="w-full mt-6 bg-slate-900 hover:bg-slate-800 text-white h-12 rounded-xl shadow-lg shadow-slate-900/20 text-base font-medium">
                                    ติดต่อสอบถาม
                                </Button>
                            </CardContent>
                        </Card>
