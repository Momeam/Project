...
import { MapPin, Bed, Bath, Ruler, Search, Heart } from 'lucide-react';
import AdBanner from '@/components/AdBanner';

export default function HomePage() {
    // --- 1. State สำหรับตัวกรองต่างๆ ---
    ...
    // --- 2. ดึงข้อมูลและ Action จาก Store ---
    // ปรับให้ดึง fetchProperties และ isLoading มาด้วย
    const { properties, fetchProperties, isLoading, toggleFavorite } = usePropertyStore();

    // --- 3. สั่งให้โหลดข้อมูลจาก Backend ทันทีที่เปิดหน้าเว็บ ---
    useEffect(() => {
        fetchProperties(); // วิ่งไปหา http://localhost:5000/api/properties
    }, []);

    // --- 4. ตรรกะการคัดกรองข้อมูล (Filter Logic) ---
    ...
    return (
        <div className="min-h-screen bg-slate-50/50 pb-20">
            {/* โซน Hero & Search Box (เหมือนเดิม) */}
            <div className="relative bg-slate-900 py-24 lg:py-32 overflow-hidden">
                ...
                <div className="container mx-auto px-4 relative z-10 text-center">
                    ...
                    <div className="bg-white p-4 rounded-2xl shadow-2xl max-w-5xl mx-auto space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                            ...
                            <select value={minBed} onChange={(e) => setMinBed(parseInt(e.target.value))} className="h-[50px] px-4 rounded-xl bg-gray-50 border border-transparent focus:bg-white focus:border-slate-200 text-sm cursor-pointer outline-none">
