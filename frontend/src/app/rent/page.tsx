...
import { MapPin, Bed, Bath, Ruler, Search, ArrowLeft } from 'lucide-react';
        ...
        <div className="container mx-auto px-4 max-w-7xl">
            ...
            <div className="bg-gray-50 dark:bg-slate-900/60 backdrop-blur-xl p-8 rounded-3xl shadow-lg dark:shadow-2xl border border-gray-200 dark:border-slate-800 mb-10">
                <h1 className="text-3xl font-bold mb-6 text-slate-900 dark:text-white tracking-tight">
                    ค้นหาอสังหาฯ สำหรับ <span className="text-blue-600 dark:text-blue-400">"เช่า"</span>
                </h1>
                <div className="relative">
                    ...
                    <Input 
                        ...
                        className="pl-12 h-14 text-lg bg-white dark:bg-slate-950/50 border-gray-300 dark:border-slate-700 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 rounded-2xl focus-visible:ring-blue-500/50"
                    />
                </div>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-4 text-right font-medium">
                    พบประกาศ <span className="text-blue-600 dark:text-blue-400 font-bold">{filteredProperties.length}</span> รายการ
                </p>
            </div>
                ...
                {filteredProperties.length > 0 ? (
                    filteredProperties.map((property) => (
                        <Link href={`/listings/${property.id}`} key={property.id} className="group">
                            <Card className="overflow-hidden border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 backdrop-blur-sm shadow-md dark:shadow-xl hover:shadow-blue-500/10 hover:border-blue-500/30 transition-all duration-500 h-full group-hover:-translate-y-2">
                                <div className="aspect-video bg-gray-100 dark:bg-slate-800 relative overflow-hidden">
                                    ...
                                    <span className={`absolute top-4 right-4 backdrop-blur-md text-white text-xs font-bold px-4 py-2 rounded-full shadow-lg border border-white/20 ${
                                        ...
                                        {property.status === 'SOLD' ? '🤝 เช่าแล้ว' : 
                                         property.status === 'BOOKED' ? '📅 จองแล้ว' : 
                                         'เช่า'}
                                    </span>
                                </div>
                                <CardContent className="p-6">
                                    <h3 className="font-bold text-xl line-clamp-1 mb-3 text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                        {property.title}
                                    </h3>
                                    <div className="flex items-baseline gap-1 mb-4">
                                        <p className="text-blue-600 dark:text-blue-400 font-bold text-2xl tracking-tight">฿{property.price.toLocaleString()}</p>
                                        <span className="text-sm text-slate-500 dark:text-slate-400 font-medium ml-1">/ เดือน</span>
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center pt-5 border-t border-gray-100 dark:border-slate-800 text-sm text-slate-600 dark:text-slate-300 font-medium">
                                        <span className="flex items-center gap-1.5"><Bed className="w-4 h-4 text-blue-500/70"/> {property.bedrooms} นอน</span>
                                        <span className="flex items-center gap-1.5"><Bath className="w-4 h-4 text-blue-500/70"/> {property.bathrooms} น้ำ</span>
                                        <span className="flex items-center gap-1.5"><Ruler className="w-4 h-4 text-blue-500/70"/> {property.size} ตร.ม.</span>
                                    </div>
                                </CardContent>
                            </Card>
                        </Link>
                    ...
                    <div className="col-span-full flex flex-col items-center justify-center py-24 text-slate-400 dark:text-slate-500 bg-gray-50 dark:bg-slate-900/20 rounded-3xl border-2 border-gray-200 dark:border-slate-800 border-dashed">
                        ...
                        <p className="text-xl font-medium text-slate-600 dark:text-slate-300">ไม่พบประกาศสำหรับเช่าในขณะนี้</p>
